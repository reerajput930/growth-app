...
export default function Tasks() {
  ...
  useEffect(() => {
    async function fetchAllTasks() {
      //   // e.preventDefault();
      const response = await fetch("https://growth-app-backend.onrender.com/api/miss/progress/alltask");
  ...
  async function updateTasks(task) {
    const response = await fetch("https://growth-app-backend.onrender.com/api/miss/progress/updatetask", {
      method: "PUT",
      headers: {
  ...
  async function del(e, taskId) {
    e.preventDefault()
    console.log(taskId)
    const response = await fetch("https://growth-app-backend.onrender.com/api/miss/progress/remove", {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      >
        ...
        <div className="btn--block flex">
          ...
          >
            ...
            DELETE   </button>
            <Link className="" to={"/miss_progress/miss_editprogress"} onClick={()=>{ localStorage.setItem("task._id" ,task._id) }} ><FontAwesomeIcon  className="mt-[10px] w-3 ml-1" icon={faEdit} /></Link>
          {/* <button>edit</button> */}
        </div>
      </div>
    );
  ...
  return (
    <div className="taskmanger--block  ">
      ...
      <h1 className="heading  mt-12 font-bold text-xl ">Riya's Task progress Report!</h1>
      <div className="tasks--detail   ">
        ...
        >
          ...
          <div className="task--detail add--more">
              <Link to={"/miss_progress/miss_createtodo"}>
