...
export default function Tasks() {
  ...
  useEffect(() => {
    async function fetchAllTasks() {
      //   // e.preventDefault();
      const response = await fetch(`${BASE_URL}/api/miss/progress/alltask`);
  ...
  async function updateTasks(task) {
    const response = await fetch(`${BASE_URL}/api/miss/progress/updatetask`, {
      method: "PUT",
      headers: {
  ...
  async function del(e, taskId) {
    e.preventDefault()
    console.log(taskId)
    const response = await fetch(`${BASE_URL}/api/miss/progress/remove`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
